...
export default class DB_helper {
        ...
        res => {
          if (typeof res.rows.raw()[0] === "undefined") {
            ...
            if (res.rows.raw()[0].version_id < database_version) {
              this.updateDB({ old_version: res.rows.raw()[0].version_id }).then(
                value => {
                  resolve();
                }
              );
  ...
  */
  ...
  updateLastUpdate() {
    ...
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "UPDATE Settings SET version_id = ?, lastUpdate =  CURRENT_TIMESTAMP  WHERE version_id = ?;",
        [database_version, v],
        () => {
          resolve();
        },
  ...
  setUpdateTrigger(keyId, isUpdated) {
    if (isUpdated) {
      return new Promise((resolve, reject) => {
        this.db.executeSql(
          "UPDATE Key SET updateTrigger = 0 WHERE key_id = ? ;",
          [keyId],
          () => {
            resolve();
          },
    ...
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "UPDATE Key SET updateTrigger = 1 WHERE key_id = ? ;",
        [keyId],
        () => {
          resolve();
        },
  ...
  */
      ...
      this.db.executeSql(
        ...
        result => {
          ...
          for (let i = 0; i < templist.length; i++) {
            if (
              templist[i].typeId === tempObj.typeId ||
              tempObj.typeId === null
            ) {
      ...
      this.db.executeSql(
        ...
        result => {
          templist = result.rows.raw().map((ele, index) => {
            return {
              typeId: ele.typeId,
              img: ImageConfig.getSingleSpeciesImg({
                id: ele.typeId,
                name: ele.img,
                keyId: ele.key_id
              })
  ...
  getTraitValuecombo(keyId) {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "SELECT * FROM Trait WHERE key_id=? ",
        [keyId],
        traitRet => {
          traits = traitRet.rows.raw();
          const promises = traits.map((trait, index) => {
            return Promise.all([
              this.getValueToTrait(keyId, trait.trait_id),
              this.getTraitElim(trait.trait_id),
              this.getSpeciesToTrait(trait.trait_id)
            ]).then(
              results => {
  ...
  getSpeciesToTrait(keyId, trait_id) {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "SELECT species_id FROM traitHasSP WHERE key_id=? AND trait_id =?",
        [keyId, trait_id],
        ret => {
  ...
  getValueToTrait(keyId, trait_id) {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "SELECT * FROM Value WHERE key_id=? AND trait_id =?",
        [keyId, trait_id],
        valueRet => {
  ...
  getTraitElim(trait_id) {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "SELECT value_Id FROM TraitEliminate " + " WHERE trait_id = ?",
        [trait_id],
        result => {
  ...
  getKeys() {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "SELECT * FROM Key ORDER BY title",
        [],
        result => {
          resolve(result.rows.raw());
        },
  ...
  getDownloadedKeys() {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "SELECT * FROM Key " + "WHERE keyDownloaded = 1 ORDER BY title",
        [],
        result => {
          resolve(result.rows.raw());
        },
  ...
  getSpeciesFromDb(keyId) {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "SELECT * FROM Species " + " WHERE key_id = ?",
        [keyId],
        result => {
          this.db.transaction(
            () => {
  ...
  */
  ...
  deleteObservationNumbers(keyId) {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "DELETE FROM NerbyObservation" + " WHERE key_id = ?",
        [keyId],
        () => {
          resolve();
        },
  ...
  deleteObservation(observationId) {
    return new Promise((resolve, reject) => {
      this.db.executeSql(
        "DELETE FROM UserObservation" + " WHERE userObservation_id = ?",
        [observationId],
        () => {
          this.db.executeSql(
            "SELECT * FROM UserObservation ",
  ...
  deleteKey(keyID) {
    ...
    ret.push(
      new Promise((resolve, reject) => {
        this.db.executeSql(
          "DELETE FROM TraitEliminate" + " WHERE key_id = ?",
          [keyID],
          () => {
            resolve();
          },
    ...
    ret.push(
      new Promise((resolve, reject) => {
        this.db.executeSql(
          "DELETE FROM Image" + " WHERE key_id = ?",
          [keyID],
          () => {
            resolve();
          },
    ...
    ret.push(
      new Promise((resolve, reject) => {
        this.db.executeSql(
          "DELETE FROM SpHasValue" + " WHERE key_id = ?",
          [keyID],
          () => {
            resolve();
          },
    ...
    ret.push(
      new Promise((resolve, reject) => {
        this.db.executeSql(
          "DELETE FROM Species" + " WHERE key_id = ?",
          [keyID],
          () => {
            resolve();
          },
    ...
    ret.push(
      new Promise((resolve, reject) => {
        this.db.executeSql(
          "DELETE FROM Trait" + " WHERE key_id = ?",
          [keyID],
          () => {
            resolve();
          },
  ...
  deleteKeyData(keyID) {
    ...
    this.db.executeSql(
      "UPDATE Key SET keyDownloaded = 0 WHERE key_id = " + keyID + ";"
    );
    return Promise.all(ret);
  }
}
